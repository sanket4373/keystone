...
class Auth(controller.V2Controller):
    ...
    def kerberos_service_key(self, context, auth=None):
        key_file = open(CONF.signing.kerberos_service_key, 'r')
        data = key_file.read()
        key_file.close()
        return data.rstrip('\n')
	...
	#traceback.print_stack()
	    ...
	    auth_token_data['bind'] = bind
      	...
      	auth_token_data['kerberos'] = self._get_kerberos_token(user_ref['username'], user_ref['email'])
...
###############################################################################
    def _get_kerberos_token(self, username, useremail):
	
	# PART-I
	TGT={"name": "", "User_SessionKey": "", "TimeStamp": ""}
	User_Ticket={"User_SessionKey": "", "TGT": "", "TimeStamp": ""}
	Kerberos={"User_Ticket" : "", "Service_Ticket": ""}
	# PART-II
	Service_TGT = {"name" : "", "User_Service_SessionKey" : "", "TimeStamp" : ""}
	Service_Ticket = {"name" : "", "User_Service_SessionKey" : "", "Service_TGT" : "", "TimeStamp": ""}
	...
	Ka = useremail.encode('ascii')
	Sa = username.encode('ascii')
	#print("My Ka is --> ", Ka)
	#print("My Sa is --> ", Sa)
	...
	#PART -II
	null_context=""
	Kos = self.kerberos_service_key(null_context)
	#print("My KOS Key is -> ", Kos)

	Kaos = "User_Service_SessionKey"
	Kos_key = ""
	Kaos_key = ""
...
#PART -I
	Kdc_key = cms.cms_hash_data(Kdc)
	#print "Kdc_key is: ", Kdc_key
	Ka_key = cms.cms_hash_data(Ka)
	#print "Ka_key is: ", Ka_key
	Sa_key = cms.cms_hash_data(Sa)
	#print "Sa_key is: ", Sa_key


#PART -II
	Kos_key = cms.cms_hash_data(Kos)
	#print "Kos_key is: ", Kos_key 
	Kaos_key = cms.cms_hash_data(Kaos)
	#print "Kaos_key is: ", Kaos_key 
...
#PART-I
	TGT['name'] = "Alice"
	TGT['User_SessionKey'] = Sa_key
	TGT['TimeStamp'] = "T"

	User_Ticket['User_SessionKey'] = Sa_key
	User_Ticket['TimeStamp'] = "T"
#Part-II
	Service_TGT['User_Service_SessionKey'] = Kaos_key
	Service_TGT['name'] = "OpenStack"
	Service_TGT['TimeStamp'] = "T"


	Service_Ticket['User_Service_SessionKey'] = Kaos_key
	Service_Ticket['TimeStamp'] = "T"



################################################
# Now start building the individual values.
#################################################

############################################################
#Encrypting the TGT and the Entire ticket (Double encryption)
############################################################

	""" Since the values of the Ticket are in the format of a dictionary (JSON format), it cannot be encrypted directly.
	While encryption the ticket should stay in the dictionary format, that's why we use the Messagepack library. 
	Therefore we encode the Ticket using Messagepack.
	"""	

	"""
	MessagePack is a fast, compact binary serialization format, suitable for similar data to JSON. 
	This package provides CPython bindings for reading and writing MessagePack data.
	"""

	# Encrypting and encoding the TGT using msgpack
	User_Ticket['TGT'] = cms.cms_encrypt_data(TGT, Kdc_key)
	#print "Encrypted TGT ->", User_Ticket

	#print("##############################################################")

	# Encrypting and encoding the User_Ticket using msgpack
	Kerberos['User_Ticket'] = cms.cms_encrypt_data(User_Ticket, Ka_key)
	#print "Encrypted Ticket ->", Kerberos

	#print ("**************** Encrypting and Encoding Ticket for Nova************************")

	# Encrypting and encoding the Service_TGT using msgpack
	Service_Ticket['Service_TGT'] = cms.cms_encrypt_data(Service_TGT, Kos_key)
	#print "Encrypted TGT_OpenStack for OpenStack Services", Service_Ticket

	# Encrypting and encoding the Service_Ticket using msgpack
	Kerberos['Service_Ticket'] = cms.cms_encrypt_data(Service_Ticket, Sa_key)
	#print "Encrypted Ticket_OpenStack for OpenStack Services ->", Kerberos


	#print("##############################################################"
        #	 "##############################################################")

	""" Decrypting and decoding the Ticket"""
	decrypted_User_Ticket = cms.cms_decrypt_data(Kerberos['User_Ticket'], Ka_key)
	#print ("Decrypted Ticket -> ", decrypted_User_Ticket)

	#print("##############################################################")

	""" Decrypting and decoding the TGT"""
	decrypted_TGT = cms.cms_decrypt_data(User_Ticket['TGT'], Kdc_key)
	#print ("Decrypted TGT -> ", decrypted_TGT)
	#print("##############################################################")

	#print ("**************** Decrypting and Decoding Ticket for Nova************************")


	""" Decrypting and decoding the Ticket for OpenStack Services"""
	decrypted_Service_Ticket = cms.cms_decrypt_data(Kerberos['Service_Ticket'], Sa_key)
	#print ("Decrypted Ticket for OpenStack Services -> ", decrypted_Service_Ticket)

	#print("##############################################################")

	""" Decrypting and decoding the TGT"""
	decrypted_Service_TGT = cms.cms_decrypt_data(Service_Ticket['Service_TGT'], Kos_key)
	#print ("Decrypted TGT_OpSt -> ", decrypted_Service_TGT)
	#print("##############################################################")
